...
- Device-local Auto/Eco/Full performance modes. Auto becomes Eco whenever either hand tracking or a visualizer is active. Eco caps heavy drawing near 30 fps, hand inference near 20 fps, canvases at 1× resolution, camera capture near 640×360/24 fps, reduces particles/simulation grids/trails/glows, disables costly glass blur/ornamental animation, uses the CPU hand delegate, stops and releases tracking when hidden, and coordinates open copies so only one tab owns the camera. Full quality is explicit opt-in and performance mode is intentionally outside presets.
...
- Performance mode is device-local rather than patch state. The self-contained HTML layout does not inherently create GPU load; the dominant costs are MediaPipe inference, camera decoding, high-DPI canvas backing stores, visualizer simulations, blur/glow compositing, and duplicate live tabs.
...
24. Compare Auto/Eco/Full in the same scene. Confirm Auto resolves to Eco with either camera or visuals active; Eco shows 1× canvases, reduced scene populations, lower-rate hand updates, CPU hand inference, and no glass blur; hiding the tab or opening the camera in another copy releases the original tab's camera/model. Confirm audio, transport, MIDI, recording, and preset recall remain unchanged.
